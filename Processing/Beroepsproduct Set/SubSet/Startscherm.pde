...
void toonStartScherm() {  
  ...
  tekenLogo(setLogo);  
  toonKnoppenStartScherm();
}

// Functie om de achtergrond van het logo te tekenen.
void tekenLogoAchtergrond() {  
  final float ACHTERGROND_LOGO_X_POS = width * 0.2;
  final float ACHTERGROND_LOGO_Y_POS = SCHERM_HOOGTE * 0.075;
  final float ACHTERGROND_LOGO_BREEDTE = width * 0.6;
  final float ACHTERGROND_LOGO_HOOGTE = SCHERM_HOOGTE * 0.5;
...
// Functie om het logo op het startscherm te tekenen.
void tekenLogo(PImage logo) {
  final float LOGO_X_POS = width * 0.3;
  final float LOGO_Y_POS = SCHERM_HOOGTE * 0.1;
  final float LOGO_BREEDTE = width * 0.4;
  final float LOGO_HOOGTE = SCHERM_HOOGTE * 0.45; 
...
// Functie om de knoppen op het startscherm te tonen.
void toonKnoppenStartScherm() {
...
// Functie om een text in een knop weer te geven.
void tekenTextInKnopStartScherm(String text, float gridPosHoogte) {
  final int TEXT_X_POS = width / 2;
  final float TEXT_Y_POS = SCHERM_HOOGTE / 10 * gridPosHoogte;
